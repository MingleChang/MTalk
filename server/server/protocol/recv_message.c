...
Recv_message *recvMessageInit(char *id, uint32_t type, char *from, char *to, char *value) {
    Recv_message *message = malloc(sizeof(Recv_message));
    size_t length;
...
Recv_message *recvMessageFromJsonString(char *json) {
    ...
    Recv_message *message = recvMessageInit(id->valuestring, type->valueint, from->valuestring, to->valuestring, value->valuestring);
    cJSON_Delete(root);
    return message;
}
